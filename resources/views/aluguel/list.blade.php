...
@section('content')
    ...
    <div style="margin-top: 20px; margin-right: 100px; margin-left: 100px;">
        <h2>Casas para alugar</h2>

        <div class="row g-3">
            <div class="col-auto">
                <form action="{{ action('App\Http\Controllers\AluguelController@search') }}" class="row g-3"
                    method="POST">
                    @csrf
                    <div class="col-auto">
                    ...
                    <div class="col-auto">
                        <select name="tipo" id="tipo" class="form-control">
                            ...
                            <option value="locatario" @if (!empty(old('tipo')) && old('tipo') == 'locatario') selected @endif>Locatário</option>
                        </select>
                    </div>
                    <div class="col-auto">
            ...
            <div class="col-auto">
                <a class="btn btn-outline-danger"
                    href="{{ action('App\Http\Controllers\UsuarioController@PDFAluguel') }}"><i
                        class="far fa-file-pdf"></i>
                    PDF</a>
            </div>
            <div class="col-auto">
                <a href="{{ action('App\Http\Controllers\AluguelController@cadastrar') }}" class="btn btn-success"><i
            ...
            <tbody>
                @foreach ($alugueis as $item)
                    <tr>
                        ...
                        <td>R${{ $item->valor }}</td>
                        <td>
                            {{ empty($item->alugado) ? '-' : $item->usuarios->nome }}
                        </td>
                        <td><a href="{{ action('App\Http\Controllers\AluguelController@editar', $item->id) }}"
